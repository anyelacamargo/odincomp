...
In this chapter, the effects of image segmentation quality on shape descriptors values are explored. Generally, Image segmentation is the classification of all pixels in a picture into classes. A descriptions of segmentation methods used for images in this thesis has been described at Material and Methods chapter. Arabidopsis thaliana top-view pictures have been segmented either as rosettes or background (Figure 1). However, classification methods are prone to several kind of classification errors. Briefly, all pixels strictly lay on one of the classes, plants on the foreground, and all other elements in the image on the background. However, plant pixels may be not classified correctly, becoming a false negative. This may happens due to similarity in colours between plant and other image elements. Examples are yellow or grey regions on leaves, pieces of soil or stones on top of plant leaves, or even thin petioles. On the other hand, background pieces may be misclassified as foreground, being then a false positive. Examples are green pixels, from moss or algae on soil, whose colour is similar to plants, and also stones and plastic pieces, e.g. pot border, that reflect lights as plants do (Figure 2). 
Figure 1 Example of Arabidopsis thaliana Segmentation Left). Plant pixels. Center) Original Image. Right) Background pixels
